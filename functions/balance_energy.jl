...
function annual_energy_balance(consumer::Residential, pvsys::PVSystem; print_output=false)
    ...
    for (ix,m) in enumerate(days_p_month)
        ...
        for i in 1:m
           ...
           global_withdrawl += withdrawn_energy #Gastos 
           real_surplus = carry_over     
           max_surplus = min(carry_over + withdrawn_energy, global_allowance) #Der Legitimo
           carry_over = max(max_surplus - withdrawn_energy,0)  
